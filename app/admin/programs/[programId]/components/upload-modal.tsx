...
import { uploadDataMany } from "@/actions/upload-many";
...
export const ApplicantUploadModal = ({ isOpen, onClose, programId }: Props) => {
  ...
  const [dataRows, setDataRows] = useState<UploadRowSubmission[]>([]);
      ...
      reader.onload = (event) => {
        ...
        startTransition(() => {
          try {
            ...
            const dataRows = XLSX.utils.sheet_to_json<UploadRowSubmission>(
              sheet,
              { raw: false }
            );
            setProgress(100);
  ...
  const onSubmit = () => {
    ...
    startTransition(() => {
      uploadDataMany(plainRows, programId)
        .then((res) => {
          if (res.success) {
            setTimeout(() => {
              setProgress(100);
            ...
            toast.success(res.message, {
              description: (
                <p>
                  `${res.inserted} data berhasil, ${res.skipped} duplikat
                  di-skip.`
                </p>
              ),
            });
            ...
            toast.error(res.message, {
              description: <p>{res.message || "Upload gagal."}</p>,
            });
          }
        })
        .catch((err) => {
          ...
          toast.error(err?.message, {
            description: (
              <p>{err?.message || "Terjadi kesalahan saat mengunggah."}</p>
            ),
          });
        });
    });
    ...
    >
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            Upload applicants excel file in here!
          </DialogDescription>
        </DialogHeader>
        <div
          {...getRootProps()}
